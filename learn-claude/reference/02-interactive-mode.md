...
**Side questions**:
- **Full visibility**: sees the current conversation, so you can ask about code Claude has already read, decisions it made earlier, or anything else from the session
- **Ephemeral**: question and answer appear in a dismissible overlay and never enter the conversation history
